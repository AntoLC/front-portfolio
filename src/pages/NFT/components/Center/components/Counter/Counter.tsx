...
const calculIntervalLeft = (type: ECounter, setValue: React.Dispatch<React.SetStateAction<number>>) => {
  ...
  if (type === ECounter.H) setValue(Math.round(seconde / (60 * 60)));
  else if (type === ECounter.M) setValue(Math.round(seconde / 60) % 60);
  else if (type === ECounter.S) setValue((seconde + 30) % 60); // Not sure why
};
...
export const Counter = (props: CounterProps) => {
  ...
  return (
    <div className={"Counter-" + global.app_config.CSS_ID + " mx-4 text-center"}>
      ...
      <div className="text-xs">{props.typeString}</div>
    </div>
  );
};
